...
def profile_processor(request):
    if request.user.is_authenticated:
        ...
        user_alerts = Alert.objects.filter(user=user_profile).filter(status='UNREAD').order_by('-created_on')[:5]
        all_alerts = Alert.objects.filter(user=user_profile).filter(status='UNREAD').order_by('-created_on')
